...
class Multiple_text_Classifier(LComponent):
    ...
    def __init__(self):
        ...
        self.model_path = None  # temporary file/dir path
        self.classifier_code = None
    ...
    def set_file_path(self):
        ...
        # self.file_path = os.path.join(self.model_path, ran_str)
        #
        ...
        if self.temporary_dir is not None:
            ...
            # in case the last time run model, stop by accident, and did not run cleanup()
        self.temporary_dir = TemporaryDirectory(prefix=None, dir=self.model_path)
        self.file_path = self.temporary_dir.name
    ...
    def text_split(self, data, data_type):
        ...
        text_name = self.text_clomn_name
        # in case self.train_data do not have ID
        # get 'content' column text,convert to list
        text_set = data[[text_name]].values.tolist()
        # get the stopwords list
        stopwords = {}.fromkeys([line.rstrip() for line in codecs.open(os.path.join(self.root_path, "stopwords.txt"),
                                                                       'r', 'utf-8-sig')])
    ...
    def run(self):
        try:
            ...
            """---------add by wwang29----"""
            try:
                classifier_model = NamedTemporaryFile(suffix=".model.bin", dir=self.file_path)
                self.classifier = fasttext.supervised(os.path.join(self.file_path, 'fasttext_trainingset.txt'),
                                                      os.path.splitext(classifier_model.name)[0], label_prefix='__label__',
                                                      epoch=200, min_count=3, word_ngrams=3, ws=10, thread=32, lr=0.1,
                                                      dim=200, bucket=5000000)
    ...
    def __getstate__(self):
        odict = super(Multiple_text_Classifier, self).__getstate__()
        odict["train_data"] = None
        odict["test_data"] = None
        odict["temporary_dir"] = None
        odict["file_path"] = None
        odict["root_path"] = None
        odict["classifier"] = None
        return odict
    ...
    def __setstate__(self, state):
        ...
        try:
            ...
            classifier_model.write(self.classifier_code)

            self.classifier = fasttext.load_model(classifier_model.name
